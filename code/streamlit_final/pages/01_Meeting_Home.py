...
def view_meeting_result():
    ...
    with col2:
        ...
        st.text_area(
            label="",
            value=result["stt"].get("full_text", ""),
            height=500,          # 왼쪽이랑 비슷한 높이로 숫자 조절
            disabled=True        # 읽기 전용
        )
    # ---- 저장 버튼 추가 ----
    if st.button("저장하기", use_container_width=True):
        ...
        # ---- 화자 매핑된 텍스트 생성 후 세션 상태에 저장 ----
        mapped_text = []
        for seg in result["stt"]["segments"]:
            real_name = st.session_state.meeting_results[idx]["speaker_map"].get(seg["speaker"], seg["speaker"])
            mapped_text.append(f"{real_name}: {seg['text']}")

        # 결합된 텍스트를 하나의 문자열로 변환
        mapped_text_str = "\n".join(mapped_text)

        # 세션 상태에 저장
        st.session_state.mapped_text_str = mapped_text_str

    st.markdown("---")

    if st.button("요약/태스크/회의록 생성하기", use_container_width=True):
            with st.spinner("회의 요약/태스크/회의록 생성 중입니다..."):
                llm_result = run_meeting_llm()  # 화자 매핑된 텍스트를 입력으로 LLM 실행

            # 결과 저장
            st.session_state.meeting_results[idx]["llm"] = llm_result
    ...
    if "llm" in result:
        tab1, tab2, tab3, tab4 = st.tabs(["Summary", "Task", "Full Text(Mapped)", "회의록"])
        ...
        with tab1:
            ...
            for ag in result["llm"]["agendas"]:
                st.markdown(f"- **{ag['title']}**: {ag['description']}")
        ...
        with tab2:
            ...
            if not tasks:
                ...
                for t in tasks:
                    st.write(f"- **{t['assignee']}**: {t['description']} (기한: {t['due']} ~ {t['due_date']})")

        with tab3:

            st.text(st.session_state.mapped_text_str)
